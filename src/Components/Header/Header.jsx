...
const Header = ({ search }) => {
    ...
    return (
        <div className="h-16 px-2 fixed top-0 shadow-sm w-screen md:px-10 z-10 flex justify-between items-center bg-primaryGreen">
            ...
            <div className="text-center relative">
                ...
                <div className={`right-32 top-14 absolute flex flex-row whitespace-nowrap p-2 rounded-xl gap-4`}>
                    {
                        type && <Fragment>
                            <span className={`absolute shadow shadow-gray-600 ${showMenu ? `top-0` : `top-[-10rem]`} cursor-pointer transition-all duration-300 ease-linear delay-[0.03s] bg-white hover:bg-gray-700 w-32 hover:text-white rounded-md p-1 text-lightGreen`} onClick={() => naviagte("/profile")}><i className="fa fa-user"/> Profile</span>
                            <span className={`absolute shadow shadow-gray-600 ${showMenu ? `top-12` : `top-[-10rem]`} cursor-pointer transition-all duration-300 ease-linear delay-[0.05s] bg-white hover:bg-gray-700 w-32 hover:text-white rounded-md p-1 text-lightGreen`} onClick={() => type == "student" ? naviagte("/") : naviagte("/sessions/create")}>{type == "student" ? <Fragment><i className="fa fa-home"/> Home</Fragment> : <Fragment><i className="fa fa-circle-plus"/> Create</Fragment>}</span>
                            <span className={`absolute shadow shadow-gray-600 ${showMenu ? `top-24` : `top-[-10rem]`} cursor-pointer transition-all duration-300 ease-linear delay-[0.07s] bg-white hover:bg-gray-700 w-32 hover:text-white rounded-md p-1 text-lightGreen`} onClick={() => naviagte("/sessions/bookings")}> <i className="fa fa-history"/> Bookings</span>
                            <span className={`absolute shadow shadow-gray-600 ${showMenu ? `top-36` : `top-[-10rem]`} cursor-pointer transition-all duration-300 ease-linear bg-white delay-[0.09s] hover:bg-gray-700 w-32 hover:text-white rounded-md p-1 text-lightGreen`} onClick={() => naviagte("/sessions")}><i className="fa fa-list" /> { type == "mentor" ? "My " : "" }Sessions</span>
                        </Fragment>
                    }    
                    <button className={ `absolute shadow shadow-gray-600 ${showMenu ? `${type ? `top-48` : `top-0`}` : `top-[-10rem]`} ${id ? `text-red-600` : `text-[#136E61]`} transition-all duration-300 delay-[0.11s] ease-linear bg-white w-32 p-1 rounded-md px-3 hover:bg-gray-700 hover:text-white` } onClick={id ? logout : login}>{id ? <Fragment><i className="fa fa-sign-out"/> LogOut</Fragment> : <Fragment><i className="fa fa-sign-in"/> LogIn</Fragment>}</button>
                </div>
            </div>
        </div>
    )
